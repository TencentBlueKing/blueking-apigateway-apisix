...
local plugin = require("apisix.plugins.bk-stage-rate-limit")
local ratelimit = require("apisix.plugins.bk-rate-limit.init")

describe(
    "bk-stage-rate-limit", function()
        local ctx
        before_each(
            function()
                stub(
        ...
        it(
            "should wrap rate limit", function()
                assert.is_equal(plugin.priority, 17652)
                assert.is_equal(plugin.name, "bk-stage-rate-limit")
            ...
            "access()", function()
                ...
                it(
                    "should 429, conf.rates[__default] 1 rate exceed", function()
                        ...
                        assert.is_equal(ctx.var.bk_apigw_error.error.code, 1642902)
                        assert.is_equal(ctx.var.bk_apigw_error.error.code_name, "RATE_LIMIT_RESTRICTION")
                        assert.is_equal(
                            ctx.var.bk_apigw_error.error.message, "API rate limit exceeded by stage strategy"
                        )
